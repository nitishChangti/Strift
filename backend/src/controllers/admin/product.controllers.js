  ...
  try {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { products, hasMore, totalProducts },
...
const getSingleProduct = asyncHandler(async (req, res) => {
  ...
  if (
    (name && name.length !== 0 && typeof name === "string") ||
      ...
      matchingCategories.forEach((category) => {
        ...
        if (category.subCategories && category.subCategories.length > 0) {
          ...
          const matchingSubCategories = category.subCategories.filter(
            (subCat) => new RegExp(`\\b${cat}\\b`, "i").test(subCat.name)
          );
  ...
  if (
    ...
    query.$or.push({
      $and: generateRegexQuery("description", descriptionKeywords),
    });
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, Product, "product found", "true"));
});

const updateProduct = asyncHandler(async (req, res) => {
  try {
    console.log(
      "this is a update product controller of a update product route"
    );
    const mainImageFile = req.files?.image ? req.files.image[0] : null;
    const additionalImageFiles = req.files?.images || [];
    const {
    ...
    const pattern =
      /^https?:\/\/res\.cloudinary\.com\/.*\.(?:jpg|jpeg|png|gif|bmp|webp)$/i;
    ...
    if (pattern.test(req.body.images)) {
      ...
      if (findProduct.images && images && images.length > 0) {
        ...
        for (let i = 0; i < findProduct.images?.length; i++) {
          ...
          publicId = `${publicId[1]}/${publicId[2]}/${publicId[3]}`.split(
            "."
          )[0];
