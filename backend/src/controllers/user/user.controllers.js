...
const checkUserController = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { exists: true }, "User exists", true));
  } else {
    // ❌ User does not exist
    return res
      .status(404)
      .json(
        new ApiResponse(404, { exists: false }, "User does not exist", false)
      );
  }
});
...
const getUserProfileData = asyncHandler(async (req, res) => {
  try {
    ...
    const user = await User.findOne({ _id: req.user._id }).select(
      "-otp -__v -updatedAt -createdAt -role -refreshToken -address "
    );
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { user }, "User profile data", true));
  } catch (error) {
...
const userProfileAddress = asyncHandler(async (req, res) => {
  try {
    ...
    let query = {};
    if (name && typeof name === "string" && name.length > 0 && name != null)
      query.name = name;
    if (
      phoneNumber &&
      typeof phoneNumber === "string" &&
      phoneNumber.length > 0 &&
      /^\+[0-9]{1,3}[0-9\s.-]{7,15}$/.test(phoneNumber) &&
      phoneNumber != null
    )
    ...
    if (
      altNumber &&
      typeof altNumber === "string" &&
      altNumber.length > 0 &&
      altNumber != null
    )
      query.alternatePhoneNumber = altNumber;
    if (
      addressType &&
    ...
    if (addressId) {
      ...
      if (addressIndex === -1) {
        return res
          .status(404)
          .json(new ApiResponse(404, "Address not found", null, false));
      }
    ...
    if (error instanceof Error) {
      return res
        .status(400)
        .json(new ApiResponse(400, error.message, null, false));
    }
...
const delteUserAddress = asyncHandler(async (req, res) => {
  try {
    ...
    if (!addressId) {
      return res
        .status(400)
        .json(new ApiResponse(400, "Address ID is required", null));
    }
...
const searchProductDetail = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { products }, "Product found", true));
  } catch (error) {
...
const productSuggestionsForUser = asyncHandler(async (req, res) => {
  try {
    ...
    let suggestions = await product
      .find(searchQuery)
      .select("name CategoryName description price color size")
      .limit(10);
    let completeproductData = await product.find(searchQuery).limit(10);
    ...
    return res
      .status(500)
      .json({ error: "Server error: Unable to fetch product details" });
  }
});
...
const getUserProductWishlist = asyncHandler(async (req, res) => {
  ...
  if (!user.wishlist || user.wishlist.length === 0) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "Wishlist is empty", true));
  }
...
const fetchCartByUser = asyncHandler(async (req, res) => {
  try {
    ...
    const userData = await User.findById(userId).select(
      "_id firstName lastName phone address cart savedForLater"
    );
    if (!userData) {
      return res
        .status(404)
    ...
    return res.json(
      new ApiResponse(200, userData, "User data fetched successfully", true)
    );
  } catch (err) {
...
const deleteFromCartUser = asyncHandler(async (req, res) => {
  try {
    ...
    if (user.cart.length === originalCartLength) {
      return res
        .status(404)
        .json(new ApiResponse(404, null, "Product not found in cart", false));
    }
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          user.cart,
          "Product removed from user cart successfully",
          true
        )
      );
  } catch (error) {
  ...
  try {
    ...
    for (const newItem of newCartItems) {
      ...
      if (!newItem.productName) {
        console.error(
          "❌ Error: Cart item is missing the required 'productName' field:",
          newItem
        );
        return res
          .status(400)
    ...
    if (!newItemsAdded) {
      ...
      return res.json(
        new ApiResponse(200, user.cart, "Cart already up-to-date", true)
      );
    }
    ...
    return res.json(
      new ApiResponse(200, user.cart, "Cart updated successfully", true)
    );
  } catch (error) {
...
const updateToaddressForCart = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, updatedUser, "Address updated successfully", true)
      );
  } catch (err) {
    if (err instanceof Error) {
  ...
  try {
    ...
    userCart.forEach((cart) => {
      const Product = products.find(
        (p) => p && p._id.toString() === cart.productId.toString()
      );
    ...
    if (cartWithImages.length === 0) {
      return res
        .status(404)
        .json(new ApiResponse(404, null, "No products found", false));
    }
...
const saveForLaterProduct = asyncHandler(async (req, res) => {
  try {
    console.log(
      ` this is a user controller of add product from cart to saveForLater`,
      req.params
    );
    const userId = req.user._id;
    const productId = req.params.id;
    ...
    const alreadySaved = user.saveForLater.find(
      (item) => item.productId?.toString() === productId
    );
    if (alreadySaved) {
      return res
        ...
        .json(
          new ApiResponse(
            400,
            null,
            "Product already exists in save for later list",
            false
          )
        );
    }
...
const getUserSavedForLater = asyncHandler(async (req, res) => {
  try {
    ...
    if (!saveForLater || saveForLater.length === 0) {
      return res
        .status(200)
        .json(new ApiResponse(200, [], "No products saved for later", true));
    }

    return res
      .status(200)
      .json(
        new ApiResponse(200, saveForLater, "Products saved for later", true)
      );
  } catch (error) {
...
const moveToCartForUser = asyncHandler(async (req, res) => {
  try {
    ...
    const productExistingInCart = user.cart.find(
      (item) => item.productId.toString() === productId
    );
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          {
            saveForLater: updatedUser.saveForLater,
...
const placeOrder = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json(
        new ApiResponse(true, { savedOrder }, "Order placed successfully", true)
      );
  } catch (err) {
...
const fetchPendingOrderByUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, order, "Pending order fetched successfully", true)
    );
});
...
const getOrderDetails = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, order, "Order details fetched successfully", true)
      );
  } catch (error) {
...
const getHomeProducts = asyncHandler(async (req, res) => {
  ...
  const products = await product
    .find({})
    .sort({ createdAt: -1 }) // or ratings later
    .limit(limit);
