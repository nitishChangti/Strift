...
// Fetch categories from backend and populate category field
=======
...
<<<<<<< HEAD
  const stock = createForm.stock.value;
...
<<<<<<< HEAD
  if (selectedColors && selectedColors.length > 0) {
    console.log('selectedColors is:', selectedColors);
    formData.append('selectedColors', JSON.stringify(colors));
  }
  else {
    alert('Please select at least one color');
  }
  if (stock && typeof (stock) === 'string' && stock.trim() !== '' && stock.length > 0) {
  ...
  if (price && typeof (price) === 'string' && price.trim() !== '' && price.length > 0) {
